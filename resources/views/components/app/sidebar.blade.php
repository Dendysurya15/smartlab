        ...
        <div class="flex justify-around items-center  mb-10 pr-3 sm:px-2">
            ...
            <a class="block" href="{{ route('dashboard') }}">
                <div class="w-10 h-10 shrink-0 mr-2 sm:mr-3">
                    <img class="rounded-full" src="{{ asset('images/CBI-logo.png') }}" width="50" height="50" alt="Alex Shatov" />
                </div>
        ...
        <div class="space-y-8">
            ...
            <div>
                ...
                <ul class="mt-3">
                        ...
                        <a class="block text-slate-200 hover:text-white truncate transition duration-150 @if(in_array(Request::segment(1), ['dashboard'])){{ 'hover:text-slate-200' }}@endif" href="{{route('dashboard')}}">
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="shrink-0 h-6 w-6" height="1em" viewBox="0 0 512 512">
                                    ...
                                    <path class="fill-current @if(in_array(Request::segment(1), ['dashboard'])){{ 'text-emerald-500' }}@else{{ 'text-slate-600' }}@endif" d="M0 96C0 60.7 28.7 32 64 32H448c35.3 0 64 28.7 64 64V416c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V96zm64 64V416H224V160H64zm384 0H288V416H448V160z" />
                                    {{--
                                    <path
                        ...
                        <a class="block text-slate-200 hover:text-white truncate transition duration-150 @if(in_array(Request::segment(1), ['history_sampel.index','input_progress.index'])){{ 'hover:text-slate-200' }}@endif" href="#0" @click.prevent="sidebarExpanded ? open = !open : sidebarExpanded = true">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm font-medium ml-3 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">Tracking
                                        Sampel</span>
                                </div>
                                <!-- Icon -->
                    ...
                    <li class="px-3 py-2 rounded-sm mb-0.5 last:mb-0 @if(in_array(Request::segment(1), ['inbox'])){{ 'bg-slate-900' }}@endif">
                        <a class="block text-slate-200 hover:text-white truncate transition duration-150 @if(in_array(Request::segment(1), ['inbox'])){{ 'hover:text-slate-200' }}@endif" href="#0">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm font-medium ml-3 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">Stok
                                    Bahan Kimia</span>
                            </div>
                        </a>
                    </li>
                    <!-- System -->
                    @can('edit_data')
                    <li class="px-3 py-2 rounded-sm mb-0.5 last:mb-0 @if(in_array(Request::segment(1), ['system'])){{ 'bg-slate-900' }}@endif" x-data="{ open: {{ in_array(Request::segment(1), ['system']) ? 1 : 0 }} }">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    <svg class="shrink-0 h-6 w-6" viewBox="0 0 24 24">
                                        ...
                                        <path class="fill-current @if(in_array(Request::segment(1), ['system'])){{ 'text-emerald-600' }}@else{{ 'text-slate-400' }}@endif" d="m23.72 12 .229.686A.984.984 0 0 1 24 13v8a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1v-8c0-.107.017-.213.051-.314L.28 12H8v4h8v-4H23.72ZM13 0v7h3l-4 5-4-5h3V0h2Z" />
                                    </svg>
                                    {{-- <svg class="shrink-0 h-6 w-6" viewBox="0 0 24 24">
                                        <path
                                            class="fill-current @if(in_array(Request::segment(1), ['dashboard'])){{ 'text-indigo-500' }}@else{{ 'text-slate-400' }}@endif"
                                    ...
                                    <span class="text-sm font-medium ml-3 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">System
                                    </span>
                                </div>
                                <!-- Icon -->
                                <div class="flex shrink-0 ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                        ...
                        <div class="lg:hidden lg:sidebar-expanded:block 2xl:block">
                            <ul class="pl-9 mt-1 @if(!in_array(Request::segment(1), ['system.index'])){{ 'hidden' }}@endif" :class="open ? '!block' : 'hidden'">
                                ...
                                @can('view_rolemanagement')
                                <li class="mb-1 last:mb-0">
                                    <a class="block text-slate-400 hover:text-slate-200 transition duration-150 truncate @if(Route::is('system.index')){{ '!text-emerald-500' }}@endif" href="{{ route('roles') }}">
                                        <span class="text-sm font-medium lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
