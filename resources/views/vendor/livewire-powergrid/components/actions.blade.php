...
<div>
    ...
    <td class="pg-actions flex justify-start   {{ $theme->table->tdBodyClass. ' ' .$theme->table->tdActionClass }}"
        ...
        @else
        <a href="{{ route($action->route, $actionClass->parameters) }}" target="{{ $action->target }}" {{
